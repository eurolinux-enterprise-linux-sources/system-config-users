...
class userProperties:
    ...
    def on_ok_button_clicked(self, *args):
        ...
        if self.accountLockCheck.get_active():
            ...
            if self.parent.ADMIN.userIsLocked(self.userEnt) == 1:
                try:
                    self.parent.ADMIN.unlockUser(self.userEnt, nonempty=True)
                    ...
                    if len(e.args) > 0 and isinstance(e.args[0], basestring):
                        ...
                        dlgtxt = _("Unlocking the user failed because:\n\n"
                                   "%(errmsg)s") % { 'errmsg': errmsg }
                    else:
                        dlgtxt = _("Unlocking the user failed due to an unknown reason.")
                    messageDialog.show_message_dialog(dlgtxt)
                    self.ready()
                    self.accountLockCheck.set_active(True)
                    self.accountLockCheck.grab_focus()
                    # the user may need to set a password
                    self.userWinPassword.set_sensitive(True)
                    self.userWinConfirm.set_sensitive(True)
                    return
