...
class userWindow:
    ...
    def __init__(self, parent, userStore, group_clist, xml, selinuxEnabled):
        ...
        self.newUidSpinButton.set_range(0, userGroupFind.mininvalidid - 1)
        ...
        self.newGidSpinButton.set_range(0, userGroupFind.mininvalidid - 1)
    ...
    def on_userWin_ok_button_clicked(self, *args):
        ...
        except userGroupFind.IdSpaceExceededError, ide:
            # the UI doesn't allow too high ids, so this must be an issue of
            # not finding any free slots
            if not self.newUidCheckButton.get_active():
                msg = _("Error finding suitable user id not exceeding maximum value of %(maxid)s.")
            elif self.newGidCheckButton.get_active():
                msg = _("Error finding suitable group id not exceeding maximum value of %(maxid)s.")

            self.ready()
            messageDialog.show_error_dialog(msg %
                    { 'maxid': userGroupFind.mininvalidid - 1 })
            return
    ...
    def addToUsersGroup(self, userEnt, userName):
        ...
        try:
            ...
            messageDialog.show_error_dialog(_("The system group database cannot be read.  This problem is most likely caused by a mismatch in /etc/group and /etc/gshadow.  The program will exit now."))
            import os
            os._exit(0)
