...
from constants import (uid_min, gid_min, high_uid_ignore, high_gid_ignore,
        high_uid_ignore_numerical, high_gid_ignore_numerical)
...
if os.access("system-config-users.glade", os.F_OK):
    ...
    xml = gtk.glade.XML(
            "/usr/share/system-config-users/system-config-users.glade",
            domain=domain)
...
def url_display(url):
    ...
    if path == None:
        messageDialog.show_message_dialog(
                _("No helper application for the URL '%(url)s can be found.") %
                {'url': url})
        return
...
class mainWindow(object):
    ...
    def destroy(self, *args):
        try:
            ...
            print _("Error saving settings to %s") % self.preferences.filename
    ...
    def __init__(self, firstboot=False):
        ...
        fbutil.handle_firstboot(self.toplevel)
        ...
        self.userStore = gtk.ListStore(gobject.TYPE_STRING,
                gobject.TYPE_UINT64, gobject.TYPE_STRING, gobject.TYPE_STRING,
        ...
        col = gtk.TreeViewColumn(_("User Name"),
        ...
        col.set_resizable(True)
        self.userTreeView.append_column(col)
        col = gtk.TreeViewColumn(_("Primary Group"), gtk.CellRendererText(),
                text=2)
        col.set_sort_column_id(2)
        col.set_resizable(True)
        self.userTreeView.append_column(col)
        col = gtk.TreeViewColumn(_("Full Name"), gtk.CellRendererText(),
                text=3)
        col.set_sort_column_id(3)
        col.set_resizable(True)
        self.userTreeView.append_column(col)
        col = gtk.TreeViewColumn(_("Login Shell"), gtk.CellRendererText(),
                text=4)
        col.set_sort_column_id(4)
        col.set_resizable(True)
        self.userTreeView.append_column(col)
        col = gtk.TreeViewColumn(_("Home Directory"), gtk.CellRendererText(),
                text=5)
        ...
        self.groupTreeView.get_selection().connect("changed",
                self.itemSelected)
        self.groupTreeView.connect("row_activated", self.rowActivated)

        col = gtk.TreeViewColumn(_("Group Name"), gtk.CellRendererText(),
                text=0)
        col.set_sort_column_id(0)
        col.set_resizable(True)
        ...
        col.set_resizable(True)
        self.groupTreeView.append_column(col)
        col = gtk.TreeViewColumn(_("Group Members"), gtk.CellRendererText(),
                text=2)
        col.set_resizable(True)
        self.groupTreeView.append_column(col)
        ...
        self.filter.set_icon_from_stock(gtk.ENTRY_ICON_SECONDARY,
                gtk.STOCK_CLEAR)
        self.filter.connect('icon-press', self.on_filter_icon_pressed)

        self.filterSystemUsersGroupsCheckButton = xml.get_widget(
                'filterSystemUsersGroupsCheckButton')
        self.filterSystemUsersGroupsCheckButton.set_active(
                self.preferences['FILTER'])
        self.assignHighestUidCheckButton = xml.get_widget(
                'assignHighestUidCheckButton')
        self.assignHighestUidCheckButton.set_active(
                self.preferences['ASSIGN_HIGHEST_UID'])
        self.assignHighestGidCheckButton = xml.get_widget(
                'assignHighestGidCheckButton')
        self.assignHighestGidCheckButton.set_active(
                self.preferences['ASSIGN_HIGHEST_GID'])
        self.preferSameUIDGIDCheckButton = xml.get_widget(
                'preferSameUIDGIDCheckButton')
        self.preferSameUIDGIDCheckButton.set_active(
                self.preferences['PREFER_SAME_UID_GID'])
        ...
        selinuxEnabled = (selinux.is_selinux_enabled() > 0)

        self.userWin = userWindow.userWindow(self, self.userStore,
                self.groupTreeView, xml, selinuxEnabled)
        self.groupWin = groupWindow.groupWindow(self, self.userTreeView,
                self.groupTreeView, xml)
        self.userProperties = userProperties.userProperties(self,
                self.userTreeView, self.groupTreeView, xml, selinuxEnabled)
        self.groupProperties = groupProperties.groupProperties(self,
                self.userTreeView, self.groupTreeView, xml)
        ...
        xml.signal_connect("on_add_user_button_clicked",
                self.on_add_user_activate)
        xml.signal_connect("on_add_group_activate", self.on_add_group_activate)
        xml.signal_connect("on_add_group_button_clicked",
                self.on_add_group_activate)
        xml.signal_connect("on_properties_activate",
                self.on_properties_activate)
        xml.signal_connect("on_properties_button_clicked",
                self.on_properties_activate)
        xml.signal_connect("on_preferences_activate",
        ...
        xml.signal_connect("on_help_button_clicked",
        ...
        xml.signal_connect("on_filterSystemUsersGroupsCheckButton_toggled",
                self.on_filterSystemUsersGroupsCheckButton_toggled)
        xml.signal_connect("on_assignHighestUidCheckButton_toggled",
                self.on_assignHighestUidCheckButton_toggled)
        xml.signal_connect("on_assignHighestGidCheckButton_toggled",
                self.on_assignHighestGidCheckButton_toggled)
        xml.signal_connect("on_preferSameUIDGIDCheckButton_toggled",
                self.on_preferSameUIDGIDCheckButton_toggled)
    ...
    def refresh_users(self, filter_data):
        self.user_dict = {}
        self.gid_dict = {}
        inconsistent_users = []
        ...
        i = 0
        for user in self.get_user_list(filter_data):
            i = i + 1
    ...
    def refresh_groups(self, filter_data):
        self.group_dict = {}
        self.member_dict = {}
        inconsistent_groups = []

        i = 0
        for group in self.get_group_list(filter_data):
            i = i + 1
            if i >= self.service_interval:
                service_pending_events()
                i = 0
            ...
            if userEnt:
                ...
                try:
                    ...
                    try:
                        ...
                        try:
                            groupEnt = self.ADMIN.lookupGroupById(
                                    long(gidNumber))
    ...
    def refresh(self, *args):
        ...
        if filter_data:
            self.filter.set_icon_sensitive(gtk.ENTRY_ICON_SECONDARY, True)
        else:
            self.filter.set_icon_sensitive(gtk.ENTRY_ICON_SECONDARY, False)

        inconsistent_users = self.refresh_users(filter_data)
        inconsistent_groups = self.refresh_groups(filter_data)
        consistent = True

        if len(inconsistent_users):
            consistent = False
            msg_users = gettext.ngettext(
                    N_("I couldn't find the numerical ID "
                       "of the user '%(user)s'.\n\n"),
                    N_("I couldn't find the numerical IDs "
                       "of these users:\n%(users)s\n\n"),
        ...
        if not consistent:
            messageDialog.show_message_dialog(
                    _("%(msg_users)s%(msg_groups)s\nIn most cases this is "
                      "caused by inconsistencies in the user or group "
                      "database, e.g. between the files /etc/passwd, "
                      "/etc/group and their respective shadow files "
                      "/etc/shadow and /etc/gshadow. I will try to ignore "
                      "these entries and continue for now, but please fix "
                      "these inconsistencies as soon as possible.") % {
                        'msg_users': msg_users,
    ...
    def populate_user_list(self):
        ...
        for user in self.user_dict.keys():
            ...
            # Convert the user fullName into unicode so pygtk2 doesn't
            # truncate strings
            gecos = unicode(gecos, encoding='utf-8', errors='replace')

            if self.preferences['FILTER'] == True:
                # display users with UIDs >= UID_MIN
                if long(uidNumber) >= uid_min and not (
                        uid in high_uid_ignore and
                        long(uidNumber) in high_uid_ignore_numerical):
    ...
    def populate_group_list(self):
        ...
        for group in self.group_dict.keys():
            ...
            if groupEnt.get(libuser.GIDNUMBER) != []:
                gid = groupEnt.get(libuser.GROUPNAME)[0]
                gidNumber = groupEnt.get(libuser.GIDNUMBER)[0]
                members = self.member_dict[group]
                if not members:
                    members = []

                # concatenate the list of members in the group into a comma
                # separated list
                memberlist = string.join(members, ", ")

                if self.preferences['FILTER'] == True:
                    # display groups with GIDs > GID_MIN
                    if long(gidNumber) >= gid_min and not (
                            gid in high_gid_ignore and
                            gidNumber in high_gid_ignore_numerical):
                        iter = self.groupStore.append()
                        self.groupStore.set_value(iter, 0, gid)
                        self.groupStore.set_value(iter, 1, gidNumber)
                        self.groupStore.set_value(iter, 2, memberlist)
                        self.groupStore.set_value(iter, 3, groupEnt)
                else:
                    # display all groups
                    if long(gidNumber) > -1L:
                        iter = self.groupStore.append()
                        self.groupStore.set_value(iter, 0, gid)
                        self.groupStore.set_value(iter, 1, gidNumber)
                        self.groupStore.set_value(iter, 2, memberlist)
    ...
    def on_about_button_clicked(self, *args):
        copyright_by = [["2001 - 2013", "Red Hat, Inc."],
                        ["2001 - 2004", "Brent Fox", "bfox@redhat.com"]]
        authors = [["Brent Fox", "bfox@redhat.com"],
                   [u"Tomáš Mráz", "tmraz@redhat.com"],
                   ["Nils Philippsen", "nils@redhat.com"],
                   [u"Miloslav Trmač", "mitr@redhat.com"]]
        ...
        for cb in copyright_by:
            ...
            try:
                email = cb[2]
                cb_strings.append(
                        _("Copyright © %(years)s %(holder)s <%(email)s>") %
                        {'years': years, 'holder': holder, 'email': email})
            except IndexError:
                cb_strings.append(_("Copyright © %(years)s %(holder)s") %
                        {'years': years, 'holder': holder})
        ...
        for au in authors:
            ...
            try:
                email = au[1]
                au_strings.append(_("%(author)s <%(email)s>") %
        ...
        aboutDialog.set_website(
                "http://fedoraproject.org/wiki/SystemConfig/users")
    ...
    def on_help_button_clicked(self, *args):
        help_pages = [
                'ghelp:system-config-users#s2-system-config-users-user-new',
    ...
    def on_delete_activate(self, *args):
        ...
        if page == 0:
            if self.userTreeView.get_selection().get_selected():
                ...
                if uid == 0:
                    dlg = gtk.MessageDialog(None, 0,
                            ...
                            _("Deleting the root user is not allowed."))
                ...
                if (ts.dbMatch("basenames", homeDir).count() > 0 or
                        ts.dbMatch("basenames",
                            os.path.abspath(homeDir)).count() > 0 or
                        ts.dbMatch("basenames",
                            os.path.realpath(homeDir)).count() > 0):
                    errMsgs.append(_("- An installed software package "
                                     "contains this directory."))
                if (uid < uid_min or
                        (user == "nfsnobody" and (long(uid) == 65534L or
                            long(uid) == 4294967294L))):
                    errMsgs.append(_("- A system user owns this directory "
                                     "and removing it may impair the system's "
                                     "integrity."))
                if not os.access(homeDir, os.W_OK):
                    errMsgs.append(_("- This directory doesn't exist or isn't "
                                     "writable."))
                elif os.lstat(homeDir).st_uid != uid:
                    errMsgs.append(_("- The user '%s' doesn't own this "
                                     "directory.") % (user))

                pipe = os.popen("/usr/bin/pgrep -fl -U %d; "
                                "/usr/bin/pgrep -u %d" % (uid, uid))
                processes = pipe.readlines()
                pipe.close()
                processes_running_text = ""
                if len(processes) > 0:
                    processes_running_text = _(
                            "<b>There are currently processes running that "
                            "are owned by '%s'!</b>  This user is probably "
                            "still logged in.  ") % (user)

                text = _("Do you really want to remove the user '%s'?") % (
                        user)
                ...
                if len(errMsgs) == 0:
                    filesDeleteText = _(
                            "Delete %(user)s's home directory ('%(homedir)s') "
                            "and temporary files.") % {
                                    'user': user, 'homedir': homeDir}
                    if (os.access(mailSpool, os.W_OK) and
                            os.lstat(mailSpool).st_uid == uid):
                        filesDeleteText = _(
                                "Delete %(user)s's home directory "
                                "('%(homedir)s'), mail spool "
                    ...
                    l_txt = (gettext.ngettext(
                        N_("I won't delete %(user)s's home directory "
                           "('%(homedir)s') for this reason:\n%(reason)s"),
                        N_("I won't delete %(user)s's home directory "
                           "('%(homedir)s') for these reasons:\n%(reason)s"),
                        len(errMsgs)) % {'user': user, 'homedir': homeDir,
                            'reason': string.join(errMsgs, "\n")})
                ...
                if rc != gtk.RESPONSE_YES:
                    ...
                    if (filesDeleteCheckButton and
                            filesDeleteCheckButton.get_active() == 1):
                ...
                if (groupEnt != None and
                        len(set(self.ADMIN.enumerateUsersByGroup(
                                    groupEnt.get(libuser.GROUPNAME)[0])) -
                            set([user])) == 0):
                    # Get the user's primary group. If there's no one besides
                    # that user in the group, delete it.

                    if groupEnt.get(libuser.GIDNUMBER)[0] >= gid_min:
            ...
            if self.groupTreeView.get_selection().get_selected():
                ...
                # Let's check and see if we are deleting a user's primary
                # group. We don't want that to happen.
                members = self.ADMIN.enumerateUsersByGroup(groupName)
                for userName in members:
                    ...
                    if (userEnt.get(libuser.GIDNUMBER)[0] ==
                        ...
                        messageDialog.show_message_dialog(
                                _("You cannot remove user '%s' from their "
                                  "primary group.") % userName)
                        return

                rc = messageDialog.show_confirm_dialog(
                        _("Are you sure you want to delete the group '%s'?") %
                        groupName)
                if rc != gtk.RESPONSE_YES:
                    return
    ...
    def itemSelected(self, *args):
        # If an item is selected, sensitize the properties and the delete
        # buttons.
        # If an item is unselected, desensitize the properties and delete
        # buttons.
        page = self.notebook.get_current_page()
    ...
    def changeNotebookPage(self, *args):
        # If the user changes a notebook page, unselect all the choices on
        # both the user and group lists.
        # This keeps the program from getting confused on which widget is
        # currently selected.
        page = self.notebook.get_current_page()
    ...
    def on_filter_icon_pressed(self, entry, icon_pos, event, *data):
    ...
    def rmhomedir(self, path):
        ...
        if (path != os.path.sep and len(pathcomps) >= 2 and
                pathcomps[1] != "dev" and os.path.lexists(path)):
